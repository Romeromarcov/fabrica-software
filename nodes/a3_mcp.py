"""Agente 3 — MCP Toolsmith. Diseña e implementa los servidores MCP."""
...
def a3_mcp(state: FabricaState) -> dict:
    task = f"""
Eres el Agente 3 — MCP Toolsmith.
...
"""
    output, cost = call_agent(
        agent_key="a3_mcp",
        agent_label="Agente 3 MCP",
        task_content=task,
        model=MODEL_A3,
        include_static=["project_context", "decision_log"],
        repo_path=state["repo_path"],
    )
    save_agent_output(state["feature_id"], "a3_mcp", output)
    return {
        "mcp_tools": output,
        "current_agent": "a3_mcp",
        "cost_entries": [cost],
    }
